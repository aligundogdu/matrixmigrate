...
func runImportAssets(cmd *cobra.Command, args []string) error {
	...
	if err := orch.ImportAssets(progress); err != nil {
		return err
	}

	// Get the output file path from state
	step := state.GetStep(migration.StepImportAssets)
	
	printSuccess(i18n.T("messages.mapping_saved", step.OutputFile))
...
func runImportMemberships(cmd *cobra.Command, args []string) error {
	...
	if err := orch.ImportMemberships(progress); err != nil {
		return err
	}
